...
class Program
{
    ...
    private static void CadastrarMateria(DatabaseHelper db)
    {
        ...
        var materia = new Materia {Nome = nome,  CargaHoraria = cargaHoraria };
        db.CadastrarMateria(materia);

        Console.WriteLine("Curso cadastrado com sucesso! Pressione qualquer tecla para continuar.");
        Console.ReadKey();
    }

    private static void ExcluirAlunos(DatabaseHelper db)
    {
        Console.WriteLine("Digite o ID do aluno para excluir");
